...
class StockScreener:
    ...
    def _evaluate_single_round(
        ...
        关键修复：模拟执行买入订单更新 portfolio 持仓，使策略能触发卖出信号。
        """
...
def print_recommendations(recommendations: list[StockRecommendation]) -> None:
    ...
    header = (
        ...
        + _pad_to_width("买入概率", col_widths["buy_prob"])
        + _pad_to_width("卖出概率", col_widths["sell_prob"])
        + _pad_to_width("买入一致性", col_widths["buy_cons"])
        + _pad_to_width("卖出一致性", col_widths["sell_cons"])
    ...
    for rank, rec in enumerate(recommendations, 1):
        ...
        if rec.buy_probability >= 50 and rec.sell_probability < 20:
            ...
            action = "偏空 🔽"
        elif rec.buy_probability >= 20 and rec.sell_probability < 20:
            action = "观望偏多"
        elif rec.sell_probability >= 20 and rec.buy_probability < 20:
            action = "观望偏空"
        else:
            action = "观望 ⏸️"

        # 概率级别标记（纯 ASCII，避免 emoji 宽度问题）
        buy_level = "**" if rec.buy_probability >= 60 else ("* " if rec.buy_probability >= 40 else "  ")
        sell_level = "**" if rec.sell_probability >= 60 else ("* " if rec.sell_probability >= 40 else "  ")

        rank_str = str(rank)
        buy_prob_str = f"{buy_level}{rec.buy_probability:5.1f}%"
        sell_prob_str = f"{sell_level}{rec.sell_probability:5.1f}%"
        buy_cons_str = f"{rec.buy_consistency * 100:5.0f}%"
        sell_cons_str = f"{rec.sell_consistency * 100:5.0f}%"
        ...
        row = (
            ...
            + _pad_to_width(buy_prob_str, col_widths["buy_prob"])
            + _pad_to_width(sell_prob_str, col_widths["sell_prob"])
            + _pad_to_width(buy_cons_str, col_widths["buy_cons"])
            + _pad_to_width(sell_cons_str, col_widths["sell_cons"])
    ...
    for rec in top_stocks:
        ...
        high_buy = [r for r in sorted_results if r.buy_signal_ratio >= 0.2]
        if high_buy:
            print("    📈 买入信号策略（信号比率≥20%）:")
            for sr in high_buy:
                print(
                    f"       - {sr.strategy_name}: "
        ...
        high_sell = [r for r in sorted_results if r.sell_signal_ratio >= 0.2]
        if high_sell:
            print("    📉 卖出信号策略（信号比率≥20%）:")
            for sr in high_sell:
                print(
                    f"       - {sr.strategy_name}: "
        ...
        low_signal = [r for r in sorted_results if r.buy_signal_ratio < 0.2 and r.sell_signal_ratio < 0.2]
        if low_signal:
    ...
    # 卖出风险提示
    high_sell_stocks = [r for r in recommendations if r.sell_probability >= 30]
    if high_sell_stocks:
        print("\n  ⚠️ 卖出风险提示:")
        for rec in high_sell_stocks:
            sell_strategies = [r for r in rec.strategy_results if r.sell_signal_ratio >= 0.2]
